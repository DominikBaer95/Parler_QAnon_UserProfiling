...
# Adopted from GitHub: https://github.com/kornosk
#
#####################################################################
...
path_output = "..\\..\\..\\Data\\output\\odds_ratio\\"
...
class LogOddsRatio:
    ...
    def __init__(self, corpus_i, corpus_j, background_corpus=None, lower_case=True, rm_stopwords=True,
        ...
        with open(str(path_output + "vocabs_i.txt"), "w", encoding="utf-8") as f:
            for k, v in self.y_i.items():
                f.write(f"{k},{v}\n")
        with open(str(path_output + "vocabs_j.txt"), "w", encoding="utf-8") as f:
            for k, v in self.y_j.items():
                f.write(f"{k},{v}\n")
        with open(str(path_output + "vocabs_alpha.txt"), "w", encoding="utf-8") as f:
        ...
        # Write to files as backup
        with open(str(path_output + "delta.txt"), "w", encoding="utf-8") as f:
            for k, v in self.delta.items():
                f.write(f"{k},{v}\n")
        with open(str(path_output + "sigma_2.txt"), "w", encoding="utf-8") as f:
            for k, v in self.sigma_2.items():
                f.write(f"{k},{v}\n")
        with open(str(path_output + "z_scores.txt"), "w", encoding="utf-8") as f:
...
def main():
    ...
    if args.save_top_words is not None and args.save_top_words > 0:
        ...
        with open(str(path_output + "top_words.txt"), "w", encoding="utf-8") as f:
            i = 0
